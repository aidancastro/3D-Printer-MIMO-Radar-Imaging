...
zgrid = linspace(0.1,0.5,Nz);
ygrid = linspace(-0.5, 0.1, Ny);
...
for kk = 1:length(printer_positions)
    ...
    prev_pos=pos;
        ...
        X = X .* (1-f_res);
        recs(:,:,kk) = X; %save 3D after removing res freqs
...
if min([length(xgrid), length(ygrid), length(zgrid)]) > 2
    ...
    filename = sprintf('Calibration_3D_%s', dateTag);
end

        pause(3)
end %% end scan loop

threeDPrinter("home");z
threeDPrinter("close");

time = toc;

save(filename, 'recs', 'xgrid','ygrid','zgrid','freq','TxRxPairs','NxN','xstep','zstep','gridCenter','printer_offsets','time');
