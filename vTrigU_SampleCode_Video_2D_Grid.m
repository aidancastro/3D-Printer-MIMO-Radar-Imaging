...
% ========= Printer scan grid (snake) ==================================
NxN=5; xstep=20; zstep=20; gridCenter=[100,100]; 
printer_positions=zeros(NxN^2,2); ctr=ceil(NxN/2); idx=1;
for row=1:NxN
    z_off=(row-ctr)*zstep;
...
nrecs = 5;
    ...
    if pos(1)==prev_pos(1)
        threeDPrinter("move",pos(1),pos(2)); pause(9);
    else
        threeDPrinter("move",pos(1),pos(2)); pause(2);
    end
    prev_pos=pos;
